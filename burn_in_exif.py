...
write_dates_parser.add_argument('--failure_csv', type=str, default='failures.csv', help='CSV to write list of failures to')
...
if args.command == 'scan_dir':
    ...
    with open(args.csv, 'r') as input_csv:
        ...
        with Pool(processes=9) as pool:
            failures = filter(lambda x: x is not None, pool.map(write_date, rows))
            with open(args.failure_csv, 'w') as failure_csv:
                csv.writer(failure_csv).writerows(failures)
